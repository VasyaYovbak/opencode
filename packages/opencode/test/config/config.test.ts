...
const wellKnownAuth = (input: {
  ...
  Layer.mock(AuthWellKnown.Service)({
    configs: () =>
      Effect.sync(() => {
        const url = input.authUrl.replace(/\/+$/, "")
        ...
        const wellKnownConfig =
          metadata && typeof metadata === "object" && "config" in metadata
            ? (metadata as { config?: unknown }).config
            : input.config
        ...
        const remoteConfig =
          metadata && typeof metadata === "object" && "remote_config" in metadata
            ? (metadata as { remote_config?: { url: string; headers?: Record<string, string> } }).remote_config
            : input.remoteConfig
        if (remoteConfig?.url) {
          const remoteUrl = remoteConfig.url.replace(/\{env:TEST_TOKEN\}/g, "test-token")
          input.seen.remote = remoteUrl
          const authorization = remoteConfig.headers?.Authorization
          if (authorization) {
            input.seen.authorization = authorization.replace(/\{env:TEST_TOKEN\}/g, "test-token")
          }
          docs.push({
            url: remoteUrl,
            source: remoteUrl,
